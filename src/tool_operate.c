                           ...
                           bool *skip)
{
  ...
  if(strcmp(config->etag_save_file, "-")) {
    ...
    int fd;
#ifdef _WIN32
    fd = open(config->etag_save_file, O_CREAT | O_WRONLY | O_APPEND | O_BINARY,
              S_IREAD | S_IWRITE);
#else
    fd = open(config->etag_save_file, O_CREAT | O_WRONLY | O_APPEND,
              S_IRUSR | S_IWUSR);
#endif
    if(fd != -1) {
                                 ...
                                 struct OutStruct *heads)
{
  ...
  if(!strcmp(config->headerfile, "%")) {
    ...
    if(!per->prev || per->prev->config != config) {
      /* Truncate file with restrictive permissions (0600) */
      int fd;
#ifdef _WIN32
      fd = open(config->headerfile, O_CREAT | O_WRONLY | O_TRUNC | O_BINARY,
                S_IREAD | S_IWRITE);
#else
      fd = open(config->headerfile, O_CREAT | O_WRONLY | O_TRUNC,
                S_IRUSR | S_IWUSR);
#endif
      if(fd != -1) {
    ...
    /* Append with restrictive permissions (0600) */
    {
      int fd;
#ifdef _WIN32
      fd = open(config->headerfile, O_CREAT | O_WRONLY | O_APPEND | O_BINARY,
                S_IREAD | S_IWRITE);
#else
      fd = open(config->headerfile, O_CREAT | O_WRONLY | O_APPEND,
                S_IRUSR | S_IWUSR);
#endif
      if(fd != -1) {
        newfile = fdopen(fd, "ab");
...
#else
    /* open file for output with restrictive permissions (0600): */
    FILE *file;
    int fd;
#ifdef _WIN32
    fd = open(per->outfile, O_CREAT | O_WRONLY | O_APPEND | O_BINARY,
              S_IREAD | S_IWRITE);
#else
    fd = open(per->outfile, O_CREAT | O_WRONLY | O_APPEND,
              S_IRUSR | S_IWUSR);
#endif
    if(fd != -1) {
      file = fdopen(fd, "ab");
