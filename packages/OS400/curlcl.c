...
main(int argsc, struct arguments *args)
{
  ...
  /* Clamp command line length to buffer size to prevent out-of-bounds read. */
  cmdlen = (size_t)args->cmdargs->len;
  if(cmdlen > sizeof(args->cmdargs->string))
    cmdlen = sizeof(args->cmdargs->string);

  /* Measure arguments size. */
  exitcode = parse_command_line(args->cmdargs->string, cmdlen,
  ...
  if(!exitcode) {
    ...
    if(!argv) {
      ...
      parse_command_line(args->cmdargs->string, cmdlen,
